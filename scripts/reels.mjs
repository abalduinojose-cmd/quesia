...
const origens = [
  ['advogada_quesiaconstancio_1782128963_3925103647524202708_50304883016.mp4', 'reel-01'],
  ['advogada_quesiaconstancio_1752581039_3677238404539687753_50304883016.mp4', 'reel-02'],
  ['advogada_quesiaconstancio_1711054867_3328891064395099275_50304883016.mp4', 'reel-03'],
];
...
for (const [arquivo, nome] of origens) {
  ...
  /* Pôster do primeiro segundo (evita quadro preto de abertura) */
  const tmp = join(raiz, 'src-assets', `${nome}-poster.png`);
  execFileSync(ffmpeg, ['-y', '-ss', '00:00:01', '-i', saida, '-frames:v', '1', tmp]);
  await sharp(tmp)
    .resize({ width: 540 })
